#!/usr/bin/perl
# $Id: BkSource.pl,v 1.6 2019/12/27 21:31:49 lam Exp lam $
...
system("$find | $sed | xargs -0 tar -cvpzf $tmpDir/$outPrefix.src.tar.gz");
print "\n";
system("ls -l $tmpDir/$outPrefix*.src.tar.gz");
print "\n"; system("date '+%A, %B %-d, %Y @ %r'"); print "\n";

print "Backing up RCS archives ...\n\n";                    # Backup RCS archives
system("$find | xargs -0 tar -cvpzf $tmpDir/$outPrefix.RCS.tar.gz");
print "\n";
system("ls -l $tmpDir/$outPrefix*.RCS.tar.gz");
print "\n"; system("date '+%A, %B %-d, %Y @ %r'"); print "\n";
