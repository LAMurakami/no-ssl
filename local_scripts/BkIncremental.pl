#!/usr/bin/perl
# $Id: BkIncremental.pl,v 1.9 2007/12/05 02:30:01 lam Exp lam $
...
our $bkRef = ''; chop($bkRef = <CONFIG>);                 # backup reference file
my $bkList = ''; chop($bkList = <CONFIG>);                          # backup list
close(CONFIG);

-e $bkRef                # Perform Backup only if reference file is found/mounted
 or die "Run time error: $0\nReference file: $bkRef NOT found!\n$!\n\n";
...
foreach my $dir (@list) {                                       # Perform Backups
    print "Backing up $dir changes ...\n\n";
    my $dirOut = $dir;$dirOut =~ s@/@\.@g;
    my $find = "find $dir -type f -newer $bkRef -print0";          # find command
